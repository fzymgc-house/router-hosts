...
- **[Ship / tooling]**: `/gsd-ship`'s ship-note commit carries a `[ci skip]` marker. On this repo that is a merge-blocker, not an optimization: `protect-main` requires `CI (Go) Complete` and `Vulnerability check` on the **head** SHA, and `ci-go.yml` triggers only on `pull_request`. A skipped head means those contexts never report and the PR sits `BLOCKED` forever. Observed on PR #404. An empty follow-up commit does **not** dispatch the workflow either — the fixing commit must change at least one file. Strip the marker from the ship note, or add `workflow_dispatch` to `ci-go.yml` so a stuck SHA can be re-dispatched.
