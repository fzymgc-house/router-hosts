    ...
    pub(super) async fn search_impl(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if let Some(ip_pattern) = &filter.ip_pattern {
                ...
                params.push(Box::new(format!("%{}%", ip_pattern)));
            }
            ...
            // appears in the tags JSON array
            // Since DuckDB doesn't support UNNEST in WHERE EXISTS subqueries,
            // we'll use a simpler approach: check if the JSON array contains the tag
            if let Some(tags) = &filter.tags {
                if !tags.is_empty() {
                    ...
                    let tag_where_clause = format!("({})", tag_conditions.join(" OR "));
                    where_clauses.push(Box::leak(tag_where_clause.into_boxed_str()));
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let mut stmt = conn
                .prepare(
                    ...
                    FROM host_events
                    WHERE CAST(EXTRACT(EPOCH FROM created_at) * 1000000 AS BIGINT) <= ?
                    "#,
                )
                .map_err(|e| StorageError::query("failed to prepare aggregate query", e))?;

            let aggregate_ids: Result<Vec<String>, duckdb::Error> = stmt
                .query_map([&at_time_micros], |row| row.get(0))
                .map_err(|e| StorageError::query("failed to query aggregate ids", e))?
                .collect();

            let aggregate_ids = aggregate_ids
                .map_err(|e| StorageError::query("failed to get aggregate ids", e))?;
            ...
            for aggregate_id_str in aggregate_ids {
                ...
                let rows = stmt
                    .query_map([&aggregate_id_str as &dyn duckdb::ToSql, &at_time_micros as &dyn duckdb::ToSql], |row| {
                        Ok((
                ...
                for row in rows {
                    ...
                    let _event_timestamp =
                        DateTime::from_timestamp_micros(event_timestamp_micros).ok_or_else(
                            || {
                                StorageError::InvalidData(format!(
                    ...
                    match event_type.as_str() {
                        "HostCreated" => {
                            ...
                            let comment = event_data.get("comment").and_then(|v| v.as_str()).map(String::from);
