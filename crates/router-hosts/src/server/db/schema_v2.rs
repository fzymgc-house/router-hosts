...
impl Database {
    ...
    fn initialize_schema(&mut self) -> DatabaseResult<()> {
        ...
        //
        // Design: First-class columns for queryability, JSON only for tags array
        self.conn
            .execute(
                r#"
                CREATE TABLE IF NOT EXISTS host_events (
                    ...
                    -- Event-specific typed columns (no old_* fields - rebuild from event history)
                    ip_address INET,
                    hostname VARCHAR,
                    comment VARCHAR,
                    tags JSON,
        ...
        // Uses first-class columns for efficient querying
        self.conn
            .execute(
                r#"
...
mod tests {
    ...
    fn test_event_table_schema() {
        ...
        // First-class typed columns (not JSON event_data)
        ...
        assert!(cols.contains(&"deleted_reason".to_string()));
        assert!(cols.contains(&"event_timestamp".to_string()));
        assert!(cols.contains(&"event_metadata".to_string()));
    }
}
