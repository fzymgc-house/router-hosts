...
// local-zone/local-data directives atomically. The file opens with a `server:`
// clause header so the directives are valid regardless of where the consumer
// places the `include:` (see GH #354); for each managed name it then emits one:
//
...
func (g *UnboundConfGenerator) FormatConf(entries []domain.HostEntry) string {
	...
	// Open a server: clause so the local-zone:/local-data: directives below sit
	// in a valid clause context regardless of where the consumer places the
	// include:. These are server:-scope options; without the header unbound
	// rejects the file at the first local-zone: when it is included at top level
	// (the documented Firewalla unbound_local/ layout). server: clauses merge, so
	// emitting it unconditionally is safe even when the file is already included
	// inside an existing server: clause. See GH #354.
