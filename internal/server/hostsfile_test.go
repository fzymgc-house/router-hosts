...
func TestRegenerate(t *testing.T) {
	...
	// Add two entries
	_, err = handler.AddHost(ctx, "192.168.1.10", "server.local", nil, []string{"web"}, nil)
	require.NoError(t, err)
	_, err = handler.AddHost(ctx, "10.0.0.1", "db.local", nil, nil, nil)
	require.NoError(t, err)
...
func TestRegenerate_InvalidPath(t *testing.T) {
	ctx := context.Background()
