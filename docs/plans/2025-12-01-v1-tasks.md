...
|------|-------|-------------|
| ULID versioning | - | Store ULIDs as versions (currently uses event_version) |
| Version check on update | - | Server returns ABORTED on version mismatch |
| Import conflict modes | - | Implement "replace" and "strict" (currently skip only) |

#### Medium Priority
| Task | Issue | Description |
|------|-------|-------------|
| Snapshot retention | - | Enforce max_snapshots and max_age_days |
| Interactive conflict handling | - | CLI diff display with retry prompt |
